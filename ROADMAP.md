...
## Phase 13.6: Notes Workspace Enhancements (Completed)

Produktivitäts-Upgrade des Notiz-Workspaces mit Fokus auf Priorisierung, Wiederauffindbarkeit und unterbrechungsfreies Schreiben.

- [x] **Pin & Archive Workflow:**
  - Notizen können angepinnt/gelöst sowie archiviert/entarchiviert werden.
  - Sortierung aktiv: `pinned DESC`, danach `updatedAt DESC`.
- [x] **Auto-Save Experience:**
  - Debounced Auto-Save im Editor mit sichtbarem Save-Status (`saving/saved/error`).
- [x] **Image Integration:**
  - Bild-Upload direkt aus dem Notiz-Editor (`/api/upload`) mit automatischer Markdown-Einfügung.
- [x] **Volltextsuche:**
  - Suche über Titel und Content, inkl. Umschaltung zwischen aktiven und archivierten Notizen.
- [x] **Data Layer Hardening:**
  - Flyway-Migration für `pinned`, `archived`, `updated_at`.
  - Dexie-Offline-Schema auf neue Felder erweitert.
